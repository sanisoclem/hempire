...
handleCommand = \case
  OnboardCustomerCommand cmd ->
    runError @CrmDomainError (onboardCustomer cmd) >>= \case
      Left (_, err) -> logWarn "crm.command.onboard-customer.failed" [("error", toJSON (show err))]
      Right cid     -> logInfo "crm.command.onboard-customer.ok"     [("customerId", toJSON cid)]
  CreateInviteCommand cmd ->
    runError @CrmDomainError (createInvite cmd) >>= \case
      Left (_, err) -> logWarn "crm.command.create-invite.failed" [("error", toJSON (show err))]
  ...
  DeleteCustomerInviteCommand DeleteCustomerInvite{inviteId = iid} ->
    runError @CrmDomainError (deleteCustomerInvite iid) >>= \case
      Left (_, err) -> logWarn "crm.command.delete-invite.failed" [("error", toJSON (show err))]
  ...
  DeactivateCustomerCommand DeactivateCustomer{customerId = cid} ->
    runError @CrmDomainError (deactivateCustomer cid) >>= \case
      Left (_, err) -> logWarn "crm.command.deactivate-customer.failed" [("error", toJSON (show err))]
