...
runIdGenReal = interpret $ \_env -> \case
  NewIdRaw -> liftIO $ T.filter (/= '-') . T.pack . UUID.toString <$> UUID.V4.nextRandom
  DeriveIdRaw t -> pure $ base58Check (SHA256.hash (encodeUtf8 t))
...
base58Check payload =
  let chk = BS.take 4 (SHA256.hash (SHA256.hash payload))
   ...
   in decodeUtf8 (BS.replicate leadingZeros 49 <> encoded)
